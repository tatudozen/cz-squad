import { COPY_CONSTRAINTS } from '../schemas/copy-request.js';
/**
 ...
 */
function generateCopyContent(copy_type, businessName, targetAudience, differentiators) {
 ...
 */
function validateCopy(generated_copy, brandProfile, copy_type) {
    ...
    const includedKeywords = brandProfile.voice_guidelines?.keywords_to_use.filter((kw) => copyLower.includes(kw.toLowerCase()));
    // Check forbidden keywords
    const forbiddenWordsPresent = brandProfile.voice_guidelines?.keywords_to_avoid.some((kw) => copyLower.includes(kw.toLowerCase()));
    ...
    if (includedKeywords.length >= 2)
        confidence += 0.1;
    if (!forbiddenWordsPresent)
        confidence += 0.1;
    if (isWithinLength)
        confidence += 0.1;
    if (respectsTone)
 ...
 */
export async function generateCopy(businessName, targetAudience, differentiators, copy_type, brandProfile) {
...
//# sourceMappingURL=copywriting-service.js.map